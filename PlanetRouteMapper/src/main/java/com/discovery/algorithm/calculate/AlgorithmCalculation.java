...
public class AlgorithmCalculation {

     DBConnetionJDBC derbyDB = new DBConnetionJDBC();
    ...
    public String calculateDistance(String source, String distination) {
        ...
        List<Routes> r = new ArrayList<Routes>();
        List<Graph.Edge> edgeList = new ArrayList<Graph.Edge>();
        r = derbyDB.getList();
        for (Routes route : r) {
            Graph.Edge edgeobj = new Graph.Edge(route.getPlanetorigin(), route.getPlanetdestination(), route.getDistance());
            edgeList.add(edgeobj);
        }
        Graph.Edge[] stockArr = new Graph.Edge[r.size()];
